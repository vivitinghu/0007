...
st.markdown("""
    <style>
        ...
        .stRadio div[role="radiogroup"] {
            display: flex; /* 讓選項水平排列 */
            justify-content: space-around; /* 均勻分佈 */
            padding-bottom: 10px; /* 底部間距 */
            border-bottom: 1px solid #ffccd5; /* 導覽下方線條 */
            margin-bottom: 20px;
        }
...
def get_greeting():
    ...
    if 5 <= current_hour <= 10: # 早上 5 點到上午 10 點
        ...
        return "午安！享用美味午餐了嗎？🥗"
    elif 14 <= current_hour <= 17: # 下午 2 點到下午 5 點
        return "下午好！來點健康小點心補充能量吧！🍎"
    else: # 晚上 6 點到凌晨 4 點
...
def settings_page():
    ...
    with st.expander("✨ 新增/管理食物選項", expanded=True): # 預設展開
        ...
        gemini_api_key = "AIzaSyAC6l7WtS00C6NbfIieSP88iS4BG6P0gog" 
        ...
        st.markdown("---")
        st.markdown("#### 📝 手動新增食物")
        st.write("您也可以手動輸入食物資訊來擴充您的食物資料庫。")

        new_food_name = st.text_input("輸入新食物名稱", key="settings_new_food_name_input", help="例如：滷雞腿便當")
        # 讓使用者選擇食物類別
        new_food_category = st.selectbox("選擇食物類別", 
                                        options=["中式", "西式", "甜點", "水果", "蔬菜", "穀物與澱粉", "堅果與種子", "肉類與海鮮", "乳製品與蛋", "飲料", "其他", "AI分析"], # 新增AI分析類別
                                        key="settings_new_food_category_select", help="為您的新食物選擇一個分類")
        new_food_calories = st.number_input("輸入新食物熱量 (大卡)", min_value=0, value=0, key="settings_new_food_calories_input", help="此食物的預估熱量值")
        if st.button("新增食物到選單", key="settings_add_food_button"):
            if new_food_name and new_food_calories > 0:
                if new_food_name not in st.session_state.food_database:
                    st.session_state.food_database[new_food_name] = {"calories": new_food_calories, "category": new_food_category}
                    st.success(f"已新增 '{new_food_name}' 到食物選單，類別: {new_food_category}。未來在新增紀錄時就能選擇囉！")
                else:
                    st.warning(f"食物 '{new_food_name}' 已存在，將更新熱量為 {new_food_calories} 大卡，類別為 {new_food_category}。")
                    st.session_state.food_database[new_food_name] = {"calories": new_food_calories, "category": new_food_category}
            else:
        ...
        st.dataframe(pd.DataFrame(list(st.session_state.exercise_calories_per_min.items()), columns=['運動', '每分鐘消耗卡路里']), use_container_width=True, key="settings_exercise_database_df") # 添加 key

        exercise_to_delete = st.selectbox("選擇要刪除的運動", [""] + sorted(list(st.session_state.exercise_calories_per_min.keys())), key="settings_delete_exercise_selectbox", help="選擇您想從列表中移除的運動")
        if st.button("刪除選定運動", key="settings_delete_delete_exercise_button"):
...
page_names = ["🏠 首頁", "✍️ 新增紀錄", "📊 數據分析", "🗂 歷史紀錄", "⚙️ 設定"]
...
selected_page_index_from_radio = st.radio(
    "導覽", 
    options=page_names, 
    index=st.session_state.current_page_index, # 直接使用整數索引
    horizontal=True, 
